...
def config_parser():
    ...
    parser.add_argument("--final_height", type=int, default=272,
    ...
    with torch.no_grad():
        ...
        for img_i in i_train:
            ...
            for camera_i in range(0, 12):
                ...
                c2w = poses[camera_i]
                ret = render_test(img_idx_embed, 0, False,
                             num_img, 
                             H, W, focal, 
            ...
            smean_psnr = single_psnr / single_count
            smean_ssim = single_ssim / single_count
            smean_lpips = single_lpips / single_count
            print('At view', img_i)
            print(smean_psnr, smean_ssim, smean_lpips)
            f_i = os.path.join(basedir, expname, 'test_result2.txt')
            with open(f_i, 'a') as file:
                file.write('####### At view=' + str(img_i) + '\n')
                file.write('mean_psnr:' + str(smean_psnr) + ' mean_ssim:' + str(smean_ssim) + ' mean_lpips:' + str(
                    smean_lpips) + '\n')
        ...
        f_i = os.path.join(basedir, expname, 'test_result.txt')
        with open(f_i, 'a') as file:
